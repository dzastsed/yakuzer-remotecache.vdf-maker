...
def write_remcache_file(vdf, filepath):
    ...
    if filepath.name == "data.sys": #credits to chatgpt for the solution
        vdf_write(vdf, 1, "system/data.sys")
    elif filepath.name == "data.sav": #credits to chatgpt for dying
        save_path = filepath.parent
        vdf_write(vdf, 1, f"{save_path.name}/{filepath.name}") 
    elif filepath.name.startswith("save") and filepath.name.endswith("_icon0.dds"):
        icon_path = filepath.parent
        vdf_write(vdf, 1, f"{icon_path.name}/{filepath.name}")   
    else:
...
def write_remcache( remcache_path, data_path ): 
    with open( remcache_path, "w", newline='\n' ) as vdf:
        vdf_write( vdf, 0, "1388590" )
        vdf_write( vdf, 1, "ChangeNumber", 0 ) 
        vdf_write( vdf, 1, "ostype", 16 )

        for f in data_path.glob( "**/data.sys" ):
            write_remcache_file( vdf, f )

        for f in data_path.glob( "**/data.sav" ):
            write_remcache_file( vdf, f )

        for f in data_path.glob( "**/save*_icon0.dds" ):
            write_remcache_file( vdf, f )
...
def main():
    ...
    #files = [ f for f in save_path.glob( "savegame*.sav" ) if f.is_file() ]
    #if len( files ) == 0:
